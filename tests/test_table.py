...
def test_parse_extraction_valid_json():
    ...
    '''
    active = [
        make_response("gpt-4o"),
        make_response("claude-sonnet-4-6"),
        make_response("gemini-1.5-pro"),
    ]
    claims = parse_extraction(json_text, active)
    assert len(claims) == 1
    assert claims[0]["claim"] == "PostgreSQL is best for writes"
    assert claims[0]["positions"]["gpt-4o"] == "AGREE"


def test_parse_extraction_strips_markdown_fences():
    json_text = '```json\n{"claims": []}\n```'
    active = [make_response("gpt-4o")]
    claims = parse_extraction(json_text, active)
    assert claims == []
...
def test_build_extraction_prompt_contains_question():
    round1 = [make_response("gpt-4o", "PostgreSQL")]
    debate = [make_response("gpt-4o", "I agree")]
